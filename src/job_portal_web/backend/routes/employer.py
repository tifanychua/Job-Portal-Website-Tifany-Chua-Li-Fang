...
@router.post(
    "/confirm-edit-job/{job_id}"
)
async def confirm_edit_job(
    request: Request,
    job_id: str
):
...
async def publish_draft_job(
    ...
    # Ignore manipulated hidden input
    credit_used = (
        required_credit
    )
    ...
    db.collection(
        ...
        "description":
            (
                "Published draft job "
                f"'{edited_job.get('job_title', 'Job')}'"
            ),
