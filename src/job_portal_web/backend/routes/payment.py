...
async def payment_receipt(request: Request, order_id: str):
    ...
    if payment.get("company_id") != company_id:
        raise HTTPException(status_code=403, detail="Access denied.")
    ...
    if payment_status != "COMPLETED":
        raise HTTPException(
            status_code=400, detail=("Receipt is only available for completed payments.")
        )
    ...
    payment_method = str(payment.get("payment_method", "Card") or "Card")
...
async def download_receipt(request: Request, order_id: str):
    ...
    if completed_at:
        completed_at_display = completed_at.strftime("%d %b %Y, %I:%M %p")
    ...
    package_name = payment.get("package", "-") or "-"
    ...
    table_data = [
        ...
        ["Purchase Date", completed_at_display],
        ["Status", status],
        ["Total Paid", f"RM {amount:,.2f}"],
    ]
    ...
    return FileResponse(
        path=pdf_path, filename=f"Receipt_{order_id}.pdf", media_type="application/pdf"
    )
