...
scenarios("features/viewCompanyDetails.feature")
...
def open_details(company_id=COMPANY_ID):

    return asyncio.run(company_module.company_details(request=FakeRequest(), company_id=company_id))
...
def test_company_details_page(setup_db):

    response = open_details()

    assert response["template"] == "companyDetails.html"
...
def test_company_location(setup_db):

    response = open_details()

    assert response["context"]["company"]["location"] == "Kuala Lumpur, Kuala Lumpur, Malaysia"


def test_only_active_jobs_counted(setup_db):

    response = open_details()

    assert response["context"]["company"]["job_count"] == 2


def test_only_five_jobs_displayed(monkeypatch, applicants, companies, reviews):
    ...
    install_fake_db(monkeypatch, applicants, companies, many_jobs, reviews)

    response = open_details()

    assert response["context"]["total_jobs"] == 7

    assert len(response["context"]["jobs"]) == 5


def test_no_jobs(monkeypatch, applicants, companies, reviews):

    install_fake_db(monkeypatch, applicants, companies, {}, reviews)

    response = open_details()

    assert response["context"]["company"]["job_count"] == 0

    assert response["context"]["jobs"] == []


def test_review_average(setup_db):
...
def active_company_details(setup_db):

    pass


@given("a company has active jobs for company details")
def active_jobs_details(setup_db):

    pass


@given("a company has more than five active jobs")
def more_than_five_jobs(monkeypatch, applicants, companies, reviews):
...
def verify_location(context):
    ...
    assert (
        context.response["context"]["company"]["location"] == "Kuala Lumpur, Kuala Lumpur, Malaysia"
    )
...
def verify_five_jobs(context):

    assert len(context.response["context"]["jobs"]) == 5

    assert context.response["context"]["total_jobs"] == 7


@then("the company job count should be zero")
def verify_zero_jobs(context):

    assert context.response["context"]["company"]["job_count"] == 0


@then("inactive jobs should not contribute to the company job count")
def verify_inactive_job_excluded(context):

    assert context.response["context"]["company"]["job_count"] == 2


@then("the company rating and review count should be calculated")
def verify_review_summary(context):
...
def verify_zero_reviews(context):
...
def verify_inactive_review_excluded(context):
...
def verify_missing_applicant(context):
