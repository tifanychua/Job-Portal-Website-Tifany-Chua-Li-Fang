...
def test_update_profile_success(client):
    ...
    response = client.post("/edit-profile", data=applicant_data(), follow_redirects=False)
...
def test_upload_profile_photo_success(client):
    ...
    with open(image_path, "rb") as image:
        ...
        response = client.post(
            "/edit-profile",
            data=data,
            files={"profile_photo": ("number-1.png", image, "image/png")},
...
def test_update_without_full_name(client):
    ...
    response = client.post("/edit-profile", data=data, follow_redirects=False)
...
def test_upload_invalid_profile_photo(client):
    ...
    with open(pdf_path, "rb") as pdf:
        ...
        response = client.post(
            "/edit-profile",
            data=data,
            files={"profile_photo": ("logo.pdf", pdf, "application/pdf")},
            follow_redirects=False,
        )
...
def update_profile(context, client):
    ...
    context.response = client.post("/edit-profile", data=applicant_data(), follow_redirects=False)


@when(
    "the job seeker enters valid details such as name, contact information, education, or experience"
)
...
def validation_messages(context):
    ...
    print("✅ Scenario Passed: Validation messages displayed.")
